...
import * as routes from '@navigations/routes';
import Toast from 'react-native-simple-toast';
import { setMaintenanceInfo } from '@actions/auth/authSlice';
...
apiClient.interceptors.response.use(
  response => {
    ...
    if (response?.data?.maintenance_mode === true) {
      store.dispatch(setMaintenanceInfo(response.data));
    }
...
export const API = {
  ...
  cartApi: {
    ...
    remove_cart: (data: any) => apiClient.post(config.REMOVE_CART, data),
    payment_initiate: (data: any) =>
      apiClient.post(config.PHONEPE_INITIATE, data),
    payment_status: (merchantTransactionId: string) =>
      apiClient.get(`${config.PAYMENT_STATUS}/${merchantTransactionId}`),
    executive_cart_request_send: (data: any) =>
      apiClient.post(config.EXECUTIVE_CART_REQUEST_SEND, data),
  },
